...
func truncateAll(ctx context.Context) error {
	_, err := testPool.Exec(ctx, "TRUNCATE TABLE consumer_offsets, dead_letter_events, processing_log, processed_events, outbox_events, execution_transitions, executions CASCADE")
	return err
}
